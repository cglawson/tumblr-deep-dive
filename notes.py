...
class GetPostsThread (threading.Thread):
    def __init__(self, note_list, blog_name, note_type, offset, limit, delay):
        ...
        try:
            for post in response['posts']:
                for note in post['notes']:
                    if note['type'] == self.note_type or "both" == self.note_type:
                        if note['blog_name'] in self.note_list.keys():
                            self.note_list[note['blog_name']] += 1
                        else:
                            self.note_list[note['blog_name']] = 1
...
def getPosts(blog_name, max_posts, note_type):
    ...
    while max_posts - offset > 0:
        ...
        thread = GetPostsThread(note_list,
                                blog_name, note_type, offset, limit, delay)
        threads += [thread]
        thread.start()
...
parser.add_argument(
    "--reblogs", help="note type should include reblogs", action="store_true")
parser.add_argument(
    "--likes", help="note type should include likes", action="store_true")
parser.add_argument(
...
if args.reblogs and not args.likes:
    note_type = "reblog"
elif not args.reblogs and args.likes:
    note_type = "like"
else:
    note_type = "both"

result = getPosts(args.blog_name, args.max_posts, note_type)
